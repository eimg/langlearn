...
import { useObjects } from '@/features/content/api'
...
const QuizObjects = () => {
  ...
  const [ordered, setOrdered] = useState<ObjectItem[]>([])
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 p-6">
      <header className="space-y-2">
        <h1 className="text-2xl font-semibold text-slate-900">Object quiz</h1>
        <p className="text-slate-600">
          Look at the object, wait for the countdown, then reveal every language.
        </p>
      </header>
      ...
      {isLoading ? (
        <Card>
          <CardContent className="p-6 text-sm text-slate-600">Loading objects...</CardContent>
        </Card>
      ) : isError ? (
        <Card>
        ...
        <Card>
          <CardContent className="p-6 text-sm text-slate-600">
            No objects yet. Add some in the entry form.
          </CardContent>
        </Card>
      ) : (
        <QuizCard
